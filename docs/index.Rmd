...
$$((-\beta_1)^2-N\alpha_1^2)(\beta_2-N\alpha_2^2)\\
...
$$
\begin{aligned}
((-\beta_1)^2-N\alpha_1^2)(\beta_2^2-N\alpha_2^2)=(\beta_1\beta_2+N\alpha_1\alpha_2)^2-N(\beta_1\alpha_2+\alpha_1\beta_2)^2,\\
...
1. Find a "close" solution for some $k$:  i.e., find $a,b$ so that $Na^2 + k = b^2$
2. Then by his lemma, a solution of is $Nx^2 + k^2 = y^2$:

$$x = 2ab, \,\,y = Na^2+b^2.$$
3. Brahmagupta knew that if $k=\pm1, \pm2, \pm4$ then both $x$ and $y$ are multiples of $k$.  Hence $(x/k, y/k)$ are integral solutions of  $Nx^2 + 1 = y^2$.

## Possible Rationale ($k=4$)
...
* produces integral solutution for a $k$ smaller in absolute value
* terminates in $k = \pm1,\pm2\pm4$.

Bhaskara (1200's CE) describes this method in his work *Bijaganita*.

A.A.K Ayyangar provides a proof (1930), said to contai error.
